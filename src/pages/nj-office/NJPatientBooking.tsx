...
import { Clock, CreditCard, Building, Phone, CheckCircle2, MapPin, Navigation, Car, Train, Shield, AlertCircle, ExternalLink, Mail, Users, ArrowRight, Smartphone, Briefcase, Globe, ChevronDown, Menu, X, FileText } from "lucide-react";
...
const arrivalSteps = [
  ...
  {
    step: "02",
    title: "Bring Photo ID & Insurance",
    desc: "A valid government-issued photo ID and your insurance card are required.",
    icon: CreditCard,
  },
  {
    step: "03",
  ...
  {
    step: "04",
    title: "Check In for Your Appointment",
    desc: "Use the check-in form to notify your provider you've arrived.",
    icon: Smartphone,
  },
];

const sideNavLinksAll = [
  { label: "Find the Office", href: "#find-office", icon: MapPin },
  { label: "What to Expect on Arrival", href: "#arrival", icon: Clock },
  { label: "Need Help?", href: "#help", icon: Phone },
];

export default function NJPatientBooking() {
  const [sideNavOpen, setSideNavOpen] = useState(false);
  const [scrolled, setScrolled] = useState(false);
  const [checkInOpen, setCheckInOpen] = useState(true);
  const navigate = useNavigate();
  const sideNavLinks = sideNavLinksAll;
    ...
    <div className="min-h-screen bg-white font-body">
      ...
      <div className="hidden md:block">
        <GuidedTour
          ...
          onOpenNav={() => setSideNavOpen(true)}
          onNavigateCommunication={() => navigate("/patient/communication-examples")}
        />
      </div>
      <Helmet>
        <title>Orenda Psychiatry — Patient Portal | NJ Office Visits</title>
        <meta name="description" content="Your one-stop guide for in-person appointments at Orenda Psychiatry's New Jersey offices. Schedule visits, get directions, and check in digitally." />
      </Helmet>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-10 flex items-center justify-between h-14 md:h-16">
          <div className="flex items-center gap-3">
            <Sheet open={sideNavOpen} onOpenChange={setSideNavOpen}>
              <SheetTrigger asChild>
                <button className="p-2 -ml-2 text-foreground/60 hover:text-foreground transition-colors">
                  <Menu className="w-5 h-5" />
                </button>
              </SheetTrigger>
              <SheetContent side="left" className="w-[340px] sm:w-[400px] p-0 border-r-0 flex flex-col">
                {/* Nav header */}
                <div className="p-6 pb-4 border-b border-primary/10">
                  <img src={logo} alt="Orenda Psychiatry" className="h-7 mb-1" />
                  <p className="text-[10px] tracking-[0.25em] uppercase text-muted-foreground font-medium mt-2">Patient Portal</p>
                </div>
                ...
                <div className="flex-1 p-4 space-y-1">
                  {sideNavLinks.map((link) => (
                    <button
                      ...
                      className="w-full flex items-center gap-4 text-left px-4 py-4 rounded-xl text-foreground/80 hover:text-primary hover:bg-primary/5 transition-all duration-200 group"
                    >
                  ...
                  {/* Check-in button */}
                  <button
                    onClick={() => { setSideNavOpen(false); setCheckInOpen(true); }}
                    className="w-full flex items-center gap-4 text-left px-4 py-4 rounded-xl text-primary hover:bg-primary/5 transition-all duration-200 group mt-2"
                  >
                    <div className="w-10 h-10 rounded-xl bg-primary/15 flex items-center justify-center shrink-0">
                      <CheckCircle2 className="w-5 h-5 text-primary" />
                    </div>
                    <span className="text-sm font-bold tracking-wide">Check In</span>
                  </button>
                </div>

                {/* Patient Communication Draft Banner */}
                <div className="p-4 mt-auto">
                  <button
                    onClick={() => { setSideNavOpen(false); navigate("/patient/communication-examples"); }}
                    className="w-full rounded-2xl overflow-hidden text-left group cursor-pointer"
                    style={{ background: 'linear-gradient(135deg, hsl(270, 100%, 12%) 0%, hsl(270, 80%, 30%) 50%, hsl(270, 60%, 45%) 100%)' }}
                  >
                    <div className="relative p-6">
                      ...
                      <div className="relative">
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <span className="text-[9px] tracking-[0.3em] uppercase text-white/50 font-bold">Draft</span>
                        </div>
                        <h4 className="text-white font-display text-lg sm:text-xl font-bold leading-tight mb-2">
                          Patient Communication
                        </h4>
                        <p className="text-white/50 text-xs leading-relaxed mb-3">
                          Internal use only
                        </p>
                        <span className="inline-flex items-center gap-2 text-[10px] tracking-[0.2em] uppercase text-white/70 font-bold group-hover:text-white transition-colors">
                          Please Review <ArrowRight className="w-3 h-3 group-hover:translate-x-1 transition-transform" />
                        </span>
                      </div>
                    </div>
                  </button>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => setCheckInOpen(true)}
              className="inline-flex items-center gap-1.5 bg-primary text-primary-foreground text-[10px] tracking-[0.12em] uppercase font-bold px-4 py-2 rounded-lg hover:bg-primary/90 transition-colors">
          ...
          <div className="relative max-w-7xl mx-auto px-4 sm:px-8 md:px-14">
            <motion.div variants={fadeUp} initial="hidden" animate="visible">
              ...
              <p className="text-white/70 text-sm sm:text-base md:text-lg max-w-xl leading-relaxed mb-8">
                Find our office, check in, and prepare for your visit — everything you need in one place.
              </p>
              <div className="flex flex-col sm:flex-row gap-3 sm:gap-4">
                <button onClick={() => setCheckInOpen(true)}
                  className="inline-flex items-center justify-center gap-3 bg-white/10 border border-white/25 text-white text-[11px] sm:text-xs tracking-[0.18em] uppercase font-bold px-8 py-4 rounded-xl hover:bg-white/20 transition-all duration-300">
                  <CheckCircle2 className="w-4 h-4" /> Check In for Your Appointment
                </button>
              </div>
            </motion.div>
          </div>
      ...
      <section id="find-office" className="scroll-mt-20 relative py-16 sm:py-20 md:py-28 overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-8 md:px-14">
          <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }} className="mb-10 md:mb-14">
            <p className="text-[11px] tracking-[0.4em] uppercase text-white/40 font-medium mb-4">Directions & Parking</p>
            <h2 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl font-display font-light text-white tracking-tight">
          ...
          <div className="grid md:grid-cols-2 gap-6 md:gap-8">
            {officeCards.map((office, i) => (
              <motion.div key={office.name} variants={scaleIn} initial="hidden" whileInView="visible"
                ...
                className="bg-white/10 border border-white/15 rounded-2xl sm:rounded-3xl overflow-hidden hover:bg-white/15 transition-all duration-500"
              >
                {/* Office image */}
                ...
                <div className="p-5 sm:p-8">
                  ...
                  <div className="flex items-center gap-4 mb-5">
                    <div className="w-14 h-14 rounded-2xl bg-white/10 border border-white/10 flex items-center justify-center shrink-0">
                      <Building className="w-7 h-7 text-[hsl(270,70%,80%)]" />
                    </div>
                    <div>
                      <p className="text-white text-base font-semibold">{office.floor}</p>
                      <p className="text-white/50 text-sm">{office.address.join(", ")}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3 mb-6">
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl bg-[hsl(270,70%,80%)]/20 flex items-center justify-center shrink-0">
                        <MapPin className="w-4 h-4 text-[hsl(270,70%,80%)]" />
                      </div>
                      <p className="text-white/75 text-sm">{office.landmark}</p>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="w-9 h-9 rounded-xl bg-[hsl(270,70%,80%)]/20 flex items-center justify-center shrink-0">
                        <Navigation className="w-4 h-4 text-[hsl(270,70%,80%)]" />
                      </div>
                      <p className="text-white/75 text-sm">{office.buildingEntry}</p>
                    </div>
                  </div>

                  {/* Parking & Transit — consolidated */}
                  <div className="grid grid-cols-2 gap-4 mb-6 border-t border-white/10 pt-5">
                    <div>
                      <div className="flex items-center gap-2 mb-3">
                        <Car className="w-4 h-4 text-[hsl(270,70%,80%)]" />
                        <p className="text-white/90 text-xs font-bold uppercase tracking-wider">Parking</p>
                      </div>
                      <ul className="space-y-2">
                        {office.parking.map((p, j) => (
                          <li key={j} className="flex items-start gap-2">
                            <CheckCircle2 className="w-3 h-3 text-[hsl(270,70%,80%)] shrink-0 mt-0.5" />
                            <span className="text-white/60 text-xs leading-relaxed">{p}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
                    <div>
                      <div className="flex items-center gap-2 mb-3">
                        <Train className="w-4 h-4 text-[hsl(270,70%,80%)]" />
                        <p className="text-white/90 text-xs font-bold uppercase tracking-wider">Transit</p>
                      </div>
                      <ul className="space-y-2">
                  ...
                  <a href={office.mapLink} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-3 bg-white text-primary text-[11px] tracking-[0.2em] uppercase font-bold px-7 py-3.5 rounded-xl hover:bg-[hsl(270,70%,80%)] hover:text-white transition-all duration-500 shadow-lg">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-8 md:px-14">
          ...
          <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true }}
            ...
            <motion.button
              ...
              whileTap={{ scale: 0.99 }}
              className="w-full relative bg-foreground rounded-2xl sm:rounded-3xl p-5 sm:p-8 md:p-10 overflow-hidden text-left group cursor-pointer"
            >
              <div className="absolute inset-0 opacity-[0.04]" style={{ backgroundImage: 'radial-gradient(circle at 1px 1px, white 1px, transparent 0)', backgroundSize: '24px 24px' }} />
              <div className="absolute inset-0 bg-gradient-to-r from-transparent to-primary/20 opacity-0 group-hover:opacity-100 transition-opacity duration-500" />
              <div className="relative flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-6">
                ...
                <div className="flex-1">
                  <h3 className="text-white font-display text-xl sm:text-2xl md:text-3xl font-semibold mb-1">Check In for Your Appointment</h3>
              ...
              <div className="flex items-start gap-4 sm:gap-5">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-foreground/60 text-sm leading-relaxed mt-3">
                    During business hours (9 AM – 5 PM), a concierge is available at the front desk. After entering, complete your digital check-in so your provider knows you've arrived.
                  </p>
                </div>
              </div>
            </div>
